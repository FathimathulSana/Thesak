  ...
  try {
    ...
    if (adminData)
      return res.render("admin//add-admin", {
        layout: "admin-layout",
        msg: "admin already exist,enter a different mail id",
      });
...
exports.getAdminPanels = async function (req, res, next) {
  try {
    let delivered = await Order.find(
      { status: "delivered" },
      { status: 1, _id: 0 }
    ).lean();
    let deliveredCount = delivered.length;
    let shipped = await Order.find(
      { status: "shipped" },
      { status: 1, _id: 0 }
    ).lean();
    let shippedCount = shipped.length;
    let cancelled = await Order.find(
      { status: "cancelled" },
      { status: 1, _id: 0 }
    ).lean();
    let cancelledCount = cancelled.length;
    let placed = await Order.find(
      { status: "placed" },
      { status: 1, _id: 0 }
    ).lean();
    ...
    const eachDaySale = await Order.aggregate([
...
exports.getAdminPanel = async function (req, res, next) {
  try {
    if (!req.body.email || !req.body.password)
      return res.render("admin//admin-login", { loginerr: true });
    ...
    if (!correct)
      return res.render("admin//admin-login", { msg: "password incorrect" });
