...
module.exports = {
    ...
    getEditBanner : async function(req,res,next){
         try {
            ...
            let bannerData = await Banner.findOne({ _id : bannerId }).populate('product').lean();
            let productData = await Product.find().lean();
            res.render('admin/edit-banner', { layout : 'admin-layout' , bannerData , productData });
         } catch (error) {
            next(error)
         }
    },
    ...
    postEditBanner : async function(req,res,next){
        try {
           if(req.body.heading){
            await Banner.findOneAndUpdate({ _id : req.params.id } , { heading : req.body.heading });
           }
           if(req.body.subHeading){
            await Banner.findOneAndUpdate({ _id : req.params.id } , { subHeading : req.body.subHeading });
           }
           if(req.body.productId){
            await Banner.findOneAndUpdate({ _id : req.params.id } , { productId : req.body.productId });
           }
           if(req.body.product){
            await Banner.findOneAndUpdate({ _id : req.params.id } , { product : req.body.product})
           }
           if(req.file){
            imagePath = await Banner.findOne({ _id : req.params.id }, { _id : 0, image : 1 });
            fs.unlinkSync('public/bannerImageUploads/' + imagePath.image);
            req.body.image = req.file.filename;
            await Banner.findOneAndUpdate({ _id : req.params.id}, { image : req.body.image });
           }
         res.redirect('/admin/banner');
        } catch (error) {
            next(error)
