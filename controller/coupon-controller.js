...
module.exports = {
    ...
    postAddCoupon : async function(req,res,next){
        try {
            const couponnameExist = await Coupon.find({couponName : req.body.couponName}).lean();
            const couponcodeExist = await Coupon.find({ couponCode : req.body.couponCode }).lean();
            // if( couponnameExist[0] || couponcodeExist[0])
            // res.render('admin/add-coupon',{msg: "same coupon"});
            if(couponnameExist[0]  || couponcodeExist[0]) return res.render('admin//add-coupon',{msg:'The coupon is already exist',layout : 'admin-layout'})
 ...
 validateCoupon: async (req, res, next) => {
    try {
        userId = req.session.userId;
            couponExist = await Coupon.findOne({ couponCode: req.body.couponId, users: userId }).lean();
